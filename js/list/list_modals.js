            ...
            bodyEl.innerHTML = filteredReports.map(r => `
              <div class="detail-group-card">
                <div style="display:flex;align-items:center;justify-content:space-between;gap:.75rem;margin-bottom:.35rem;">
                  ...
                  <button type="button" class="btn btn-sm btn-primary" style="padding:.2rem .65rem;font-size:.74rem;white-space:nowrap;" onclick="window.openDetail(decodeURIComponent('${encodeInlineArg(r.fileId)}'))">Detayda Aç →</button>
                </div>
        ...
        <div style="display:flex;flex-direction:column;gap:.5rem;">
          ${analyzed.map(item => `
            <div style="background:var(--bg-surface);border:1px solid var(--border-light);border-radius:10px;padding:.8rem 1rem;display:flex;align-items:center;justify-content:space-between;gap:1rem;">
              ...
              <button class="btn btn-sm btn-primary" onclick="window.openDetail(decodeURIComponent('${encodeInlineArg(item.fileId)}'))" style="font-size:.75rem;padding:.3rem .7rem;">İncele →</button>
            </div>
          `).join('')}
        </div>
      </div>
    ...
    const bodyHtml = `
      <div style="display:flex;flex-direction:column;gap:.6rem;max-height:60vh;overflow-y:auto;">
        ${recent.map(f => {
          ...
          return `
            <div style="background:var(--bg-surface);border:1px solid var(--border-light);border-radius:10px;padding:.75rem 1rem;display:flex;align-items:center;justify-content:space-between;cursor:pointer;transition:background .15s;" onclick="window.openDetail(decodeURIComponent('${encodeInlineArg(f.id)}'))">
              <div style="min-width:0;flex:1;">
