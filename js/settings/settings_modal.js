...
window.openSettingsModal = function(initialTab = 'appearance') {
  ...
  function renderModal() {
    ...
    // ── 8. ORACLE VERİTABANI BAĞLANTILARI (TOAD STİLİ ÇOKLU YÖNETİM) ──
    else if (activeTab === 'oracle') {
      const oracleConns = window.FrpOracle ? window.FrpOracle.getConnections() : [];
      let selConnId = overlay._selectedOracleConnId || (window.FrpOracle ? window.FrpOracle.getActiveConnId() : '') || (oracleConns[0]?.id || '');
      let selConn = oracleConns.find(c => c.id === selConnId) || oracleConns[0] || {
        id: 'conn_new', alias: 'Yeni Bağlantı', host: 'localhost', port: 1521, connType: 'serviceName', serviceName: 'ORCL', sid: 'ORCL', user: '', password: ''
      };
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;gap:1rem;">
            <div>
              <div style="font-size:1.1rem;font-weight:800;color:var(--text-primary);display:flex;align-items:center;gap:.5rem;">
                <span>⚡ Toad for Oracle Bağlantı Yöneticisi</span>
                <span style="font-size:.72rem;background:rgba(217,119,6,0.15);color:var(--orange,#d97706);padding:.15rem .5rem;border-radius:6px;font-weight:800;">Çoklu Kurum & Şema</span>
              </div>
              <div style="font-size:.76rem;color:var(--text-muted);margin-top:.2rem;">
                Tüm hastane/kurum Oracle bağlantılarınızı kaydedin, detayda sorgu çalıştırırken doğrudan hedef veritabanını seçin.
              </div>
            </div>
            <button type="button" class="btn btn-sm btn-primary" id="btnAddNewOracleConn" style="font-weight:800;padding:.4rem .9rem;display:flex;align-items:center;gap:.3rem;">
              <span>➕</span><span>Yeni Bağlantı Ekle</span>
            </button>
          </div>

          <!-- Durum Bildirim Kutusu -->
          <div id="oracleSettingsAlertBox" style="display:none;padding:.65rem .9rem;border-radius:10px;font-size:.82rem;font-weight:700;"></div>

          <!-- 2 Kolonlu TOAD Arayüzü -->
          <div style="display:grid;grid-template-columns:1.3fr 1.7fr;gap:1rem;flex:1;min-height:360px;">
            
            <!-- SOL: Bağlantı Listesi Tablosu (TOAD Grid) -->
            <div style="background:var(--bg-raised);border:1.5px solid var(--border);border-radius:12px;display:flex;flex-direction:column;overflow:hidden;">
              <div style="padding:.6rem .8rem;border-bottom:1px solid var(--border-light);background:var(--bg-surface);display:flex;align-items:center;justify-content:space-between;">
                <span style="font-size:.78rem;font-weight:800;color:var(--text-secondary);">KAYITLI BAĞLANTILAR (${oracleConns.length})</span>
                <span style="font-size:.72rem;color:var(--text-muted);">Tıklayarak düzenleyin</span>
              </div>
              
              <div style="flex:1;overflow-y:auto;max-height:380px;">
                <table style="width:100%;border-collapse:collapse;font-size:.76rem;text-align:left;">
                  <thead style="position:sticky;top:0;background:var(--bg-surface);box-shadow:0 1px 2px rgba(0,0,0,.06);z-index:2;">
                    <tr>
                      <th style="padding:.5rem .6rem;color:var(--text-muted);font-weight:800;">Alias (Kurum)</th>
                      <th style="padding:.5rem .6rem;color:var(--text-muted);font-weight:800;">Kullanıcı</th>
                      <th style="padding:.5rem .6rem;color:var(--text-muted);font-weight:800;">Veritabanı</th>
                    </tr>
                  </thead>
                  <tbody>
                    ${oracleConns.map(c => `
                      <tr class="oracle-conn-row" data-id="${escHtml(c.id)}" style="cursor:pointer;border-bottom:1px solid rgba(0,0,0,.05);background:${c.id === selConn.id ? 'rgba(37,99,235,0.12)' : 'transparent'};border-left:${c.id === selConn.id ? '3.5px solid var(--accent-bright)' : '3.5px solid transparent'};">
                        <td style="padding:.55rem .6rem;font-weight:700;color:var(--text-primary);">
                          ${escHtml(c.alias || 'İsimsiz')}
                          ${c.isDefault ? '<span style="font-size:.65rem;background:#059669;color:#fff;padding:.1rem .35rem;border-radius:4px;margin-left:.3rem;font-weight:800;">VARSAYILAN</span>' : ''}
                        </td>
                        <td style="padding:.55rem .6rem;color:var(--text-secondary);font-family:var(--mono);">${escHtml(c.user || '—')}</td>
                        <td style="padding:.55rem .6rem;color:var(--text-muted);font-family:var(--mono);">${escHtml(c.host)}:${escHtml(c.port)}/${escHtml(c.connType === 'sid' ? c.sid : c.serviceName)}</td>
                      </tr>
                    `).join('')}
                  </tbody>
                </table>
            ...
            <!-- SAĞ: Seçili Bağlantı Düzenleme Formu (TOAD Login Panel) -->
            <div style="background:var(--bg-raised);border:1.5px solid var(--border);border-radius:12px;padding:1rem;display:flex;flex-direction:column;gap:.85rem;overflow-y:auto;">
              ...
              <div style="display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid var(--border-light);padding-bottom:.5rem;">
                <div style="font-weight:800;font-size:.88rem;color:var(--text-primary);display:flex;align-items:center;gap:.4rem;">
                  ...
                  <strong style="color:var(--accent-bright);">${escHtml(selConn.alias || 'Yeni Bağlantı')}</strong>
                </div>
                <div style="display:flex;gap:.3rem;">
                  <button type="button" class="btn btn-sm btn-ghost" id="btnDeleteSelectedOracleConn" style="color:#ef4444;font-size:.76rem;font-weight:700;padding:.25rem .5rem;" title="Bu bağlantıyı sil">
                    🗑️ Sil
                  </button>
                </div>
              </div>
              ...
              <!-- Form Alanları -->
              <div>
                <label style="font-size:.74rem;font-weight:800;color:var(--text-secondary);display:block;margin-bottom:.25rem;">Bağlantı Adı / Alias (Örn: Mehmet Akif Demo, Çanakkale Biga)</label>
                <input type="text" id="toadInputAlias" class="master-search-input" style="width:100%;font-size:.84rem;font-weight:700;" value="${escHtml(selConn.alias || '')}" placeholder="Kurum veya hastane adı..." />
              </div>

              <div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem;">
                <div>
                  <label style="font-size:.74rem;font-weight:800;color:var(--text-secondary);display:block;margin-bottom:.25rem;">User (Kullanıcı Adı)</label>
                  <input type="text" id="toadInputUser" class="master-search-input" style="width:100%;font-size:.84rem;font-family:var(--mono);" value="${escHtml(selConn.user || '')}" placeholder="Örn: ILKER_DINER veya SYSTEM" />
                </div>
                <div>
                  <label style="font-size:.74rem;font-weight:800;color:var(--text-secondary);display:block;margin-bottom:.25rem;">Password (Şifre)</label>
                  <input type="password" id="toadInputPassword" class="master-search-input" style="width:100%;font-size:.84rem;" value="${escHtml(selConn.password || '')}" placeholder="••••••••" />
                </div>
              </div>

              <div style="display:grid;grid-template-columns:2fr 1fr;gap:.75rem;">
                <div>
                  <label style="font-size:.74rem;font-weight:800;color:var(--text-secondary);display:block;margin-bottom:.25rem;">Host (Sunucu IP / Alan Adı)</label>
                  <input type="text" id="toadInputHost" class="master-search-input" style="width:100%;font-size:.84rem;font-family:var(--mono);" value="${escHtml(selConn.host || 'localhost')}" placeholder="10.211.208.218" />
                </div>
                <div>
                  <label style="font-size:.74rem;font-weight:800;color:var(--text-secondary);display:block;margin-bottom:.25rem;">Port</label>
                  <input type="number" id="toadInputPort" class="master-search-input" style="width:100%;font-size:.84rem;font-family:var(--mono);" value="${selConn.port || 1521}" placeholder="1521" />
                </div>
              </div>

              <div style="display:grid;grid-template-columns:1.1fr 1.9fr;gap:.75rem;">
                <div>
                  <label style="font-size:.74rem;font-weight:800;color:var(--text-secondary);display:block;margin-bottom:.25rem;">Bağlantı Tipi</label>
                  <select id="toadInputConnType" style="width:100%;padding:.42rem .6rem;border-radius:8px;border:1px solid var(--border);background:var(--bg-surface);color:var(--text-primary);font-size:.82rem;font-weight:700;">
                    <option value="serviceName" ${selConn.connType !== 'sid' ? 'selected' : ''}>Service Name</option>
                    <option value="sid" ${selConn.connType === 'sid' ? 'selected' : ''}>SID</option>
                  </select>
                </div>
                <div>
                  <label style="font-size:.74rem;font-weight:800;color:var(--text-secondary);display:block;margin-bottom:.25rem;">Service Name / SID</label>
                  <input type="text" id="toadInputDbName" class="master-search-input" style="width:100%;font-size:.84rem;font-family:var(--mono);" value="${escHtml(selConn.connType === 'sid' ? (selConn.sid || 'ORCL') : (selConn.serviceName || 'cloud'))}" placeholder="Örn: cloud, CLOUD1, XEPDB1" />
                </div>
              </div>

              <!-- İşlem Butonları -->
              <div style="display:flex;align-items:center;justify-content:space-between;gap:.5rem;margin-top:.4rem;padding-top:.75rem;border-top:1px solid var(--border-light);flex-wrap:wrap;">
                <button type="button" class="btn btn-sm btn-ghost" id="btnMakeDefaultOracleConn" style="font-size:.76rem;font-weight:700;">
                  ⭐ Varsayılan Yap
                </button>

                <div style="display:flex;gap:.45rem;">
                  <button type="button" class="btn btn-sm btn-ghost" id="btnTestCurrentToadConn" style="border:1px solid var(--border);font-weight:800;padding:.4rem .9rem;">
                    🔄 Bağlantıyı Test Et
                  </button>
                  <button type="button" class="btn btn-sm btn-primary" id="btnSaveCurrentToadConn" style="font-weight:800;padding:.4rem 1.1rem;box-shadow:0 4px 12px rgba(37,99,235,.3);">
    ...
    // ── TOAD ORACLE BAĞLANTI LİSTESİ ETKİLEŞİMLERİ ───────────────
    if (activeTab === 'oracle') {
      const getFormToadConn = () => {
        const alias = (overlay.querySelector('#toadInputAlias')?.value || '').trim() || 'Yeni Bağlantı';
        const user = (overlay.querySelector('#toadInputUser')?.value || '').trim();
        const password = overlay.querySelector('#toadInputPassword')?.value || '';
        const host = (overlay.querySelector('#toadInputHost')?.value || '').trim() || 'localhost';
        const port = parseInt(overlay.querySelector('#toadInputPort')?.value, 10) || 1521;
        const connType = overlay.querySelector('#toadInputConnType')?.value || 'serviceName';
        const dbName = (overlay.querySelector('#toadInputDbName')?.value || '').trim() || 'cloud';

        return {
          id: overlay._selectedOracleConnId || (window.FrpOracle ? window.FrpOracle.getActiveConnId() : 'conn_' + Date.now()),
          alias,
          user,
          password,
          host,
      ...
      // Tablodan bağlantı seçildiğinde
      overlay.querySelectorAll('.oracle-conn-row').forEach(row => {
        row.addEventListener('click', () => {
          overlay._selectedOracleConnId = row.dataset.id;
          if (window.FrpOracle) window.FrpOracle.setActiveConnId(row.dataset.id);
          renderModal();
        });
      });

      // Yeni Bağlantı Ekle
      overlay.querySelector('#btnAddNewOracleConn')?.addEventListener('click', () => {
        const newId = 'conn_' + Date.now();
        const newConn = {
          id: newId,
          alias: 'Yeni Kurum Bağlantısı',
          host: '10.211.208.218',
          port: 1521,
        ...
        if (window.FrpOracle) {
          window.FrpOracle.saveConnection(newConn);
          window.FrpOracle.setActiveConnId(newId);
        }
        overlay._selectedOracleConnId = newId;
        renderModal();
        safeToast('Yeni bağlantı taslağı oluşturuldu ➕', 'info');
      });

      // Seçili Bağlantıyı Kaydet
      overlay.querySelector('#btnSaveCurrentToadConn')?.addEventListener('click', () => {
        const connData = getFormToadConn();
        if (window.FrpOracle) {
          window.FrpOracle.saveConnection(connData);
          safeToast(`"${connData.alias}" bağlantısı kaydedildi! 💾`, 'success');
          renderModal();
        }
      });

      // Bağlantıyı Test Et
      overlay.querySelector('#btnTestCurrentToadConn')?.addEventListener('click', async () => {
        const connData = getFormToadConn();
        const alertBox = overlay.querySelector('#oracleSettingsAlertBox');
        const testBtn = overlay.querySelector('#btnTestCurrentToadConn');

        if (!connData.host || !connData.user || !connData.password) {
          if (alertBox) {
            alertBox.style.display = 'block';
            alertBox.style.background = '#fef2f2'; alertBox.style.border = '1px solid #fecaca'; alertBox.style.color = '#b91c1c';
            alertBox.textContent = '⚠️ Lütfen Host, User ve Password alanlarını eksiksiz giriniz.';
          }
          return;
        }

        testBtn.disabled = true;
        testBtn.textContent = 'Sınanıyor... ⏳';
        if (alertBox) alertBox.style.display = 'none';

        try {
          const res = await (window.FrpOracle ? window.FrpOracle.testConnection(connData) : fetch('/api/oracle/test-connection', {
            method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(connData)
          }).then(r => r.json()));
          ...
          if (alertBox) {
            ...
            if (res.success) {
              alertBox.style.background = '#f0fdf4'; alertBox.style.border = '1px solid #bbf7d0'; alertBox.style.color = '#15803d';
              alertBox.innerHTML = `✅ <strong>${escHtml(connData.alias)}</strong> bağlantısı başarılı!<br><span style="font-size:.76rem;opacity:.9;">${res.dbVersion || ''}</span>`;
              safeToast('Oracle bağlantı testi başarılı! ⚡', 'success');
            } else {
              alertBox.style.background = '#fef2f2'; alertBox.style.border = '1px solid #fecaca'; alertBox.style.color = '#b91c1c';
              alertBox.innerHTML = `❌ <strong>Bağlantı Hatası:</strong> ${res.reason || 'Sunucuya ulaşılamadı.'}`;
            }
          }
        } catch (e) {
          if (alertBox) {
            alertBox.style.display = 'block';
            alertBox.style.background = '#fef2f2'; alertBox.style.border = '1px solid #fecaca'; alertBox.style.color = '#b91c1c';
            alertBox.textContent = 'Bağlantı hatası: ' + e.message;
          }
        } finally {
      ...
      // Varsayılan Yap
      overlay.querySelector('#btnMakeDefaultOracleConn')?.addEventListener('click', () => {
        const connData = getFormToadConn();
        if (window.FrpOracle) {
          const all = window.FrpOracle.getConnections().map(c => ({ ...c, isDefault: c.id === connData.id }));
          window.FrpOracle.saveConnections(all);
          window.FrpOracle.setActiveConnId(connData.id);
          safeToast(`"${connData.alias}" varsayılan bağlantı yapıldı! ⭐`, 'success');
          renderModal();
        }
      });

      // Sil
      overlay.querySelector('#btnDeleteSelectedOracleConn')?.addEventListener('click', () => {
        const connData = getFormToadConn();
        window.showConfirmDialog({
          title: 'Bağlantıyı Sil',
          message: `"${connData.alias}" Oracle bağlantı profilini silmek istediğinize emin misiniz?`,
          confirmText: 'Evet, Sil',
          isDanger: true,
          onConfirm: () => {
            if (window.FrpOracle) {
              window.FrpOracle.deleteConnection(connData.id);
              overlay._selectedOracleConnId = null;
              safeToast('Bağlantı profili silindi. 🗑️', 'info');
              renderModal();
            }
          }
        });
