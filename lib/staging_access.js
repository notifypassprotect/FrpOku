...
function createStagingAccessMiddleware({ env = process.env, healthPath = '/api/health' } = {}) {
  const enabled = isTruthy(env.STAGING_ACCESS_ENABLED) || (env.NODE_ENV === 'staging' && env.STAGING_ACCESS_ENABLED !== 'false' && env.STAGING_ACCESS_ENABLED !== false && env.STAGING_ACCESS_ENABLED !== '0');
