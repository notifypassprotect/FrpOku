...
function reportIsPublic(report) {
  const payload = (report?.data && typeof report.data === 'object' && !Array.isArray(report.data)) ? report.data : {};
  return Boolean(report?.is_public || report?.isPublic || report?.inPool || report?.in_pool || payload.is_public || payload.isPublic);
}
...
function reportRowToClient(row) {
  ...
  return {
    ...
    isPublic: Boolean(row.is_public !== undefined ? row.is_public : (data.isPublic || data.is_public || data.inPool || data.in_pool)),
    ownerName: row.owner_name || data.ownerName || data.owner_name || '',
    ownerUsername: row.owner_username || data.ownerUsername || data.owner_username || '',
    ownerDepartment: row.owner_department || data.ownerDepartment || data.owner_department || '',
    sharedAt: row.shared_at || data.sharedAt || data.shared_at || null,
    deletedAt: row.deleted_at || data.deletedAt || data.deleted_at || null,
