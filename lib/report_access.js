...
function canManageReport(user, report) {
  ...
  if (!owner || owner === 'public' || owner === 'null' || owner === 'undefined') return false;
...
function buildOwnedReportRow(report, user, { ownerId = user?.id, existing = null } = {}) {
  ...
  // İstemci/Parser tarafından önceden tespit edilmiş parametre ve tabloları ekle:
  if (Array.isArray(report.tableNames)) report.tableNames.forEach(t => extractedTableNames.add(t));
  if (Array.isArray(report.paramNames)) report.paramNames.forEach(p => extractedParamNames.add(p));

  // Sorgulardan tabloları ve parametreleri ayıkla:
  if (Array.isArray(safeData.queries) && safeData.queries.length > 0) {
    safeData.queries.forEach(q => {
      ...
      // FastReport Değişken Parametreleri: <ParamName>
      const frx = /<([a-zA-Z_]\w*)>/g;
      let frm;
      while ((frm = frx.exec(sql)) !== null) {
        const pName = frm[1].toUpperCase();
        if (pName && pName.length > 1 && !/^\d+$/.test(pName) && !sqlReserved.has(pName) && !/^(FONT|COLOR|PAGE|DATE|TIME)$/i.test(pName)) {
          extractedParamNames.add(':' + pName);
        }
      }
    });
  ...
  // Eğer rawXml varsa, XML içindeki <Params> ve <Variables> bloklarını da tara:
  if (safeData.rawXml) {
    const pBlockRx = /<(?:Params|Variables)\b[\s\S]*?<\/(?:Params|Variables)>/gi;
    let pbm;
    while ((pbm = pBlockRx.exec(safeData.rawXml)) !== null) {
      const itmRx = /<item\b[^>]*?\bName=["']([^"']+)["']/gi;
      let im;
      while ((im = itmRx.exec(pbm[0])) !== null) {
        const pName = im[1].toUpperCase();
        if (pName && !sqlReserved.has(pName) && !/^(FONT|COLOR|PAGE|DATE|TIME)$/i.test(pName)) {
          extractedParamNames.add(':' + pName);
        }
      }
    }
  ...
  return {
    ...
    shared_at: sharedAt,
    deleted_at: deletedAt,
    note_html: boundedString(noteHtml, 500000),
    note_attachments: attachmentsVal,
    sql_count: sqlCount,
...
function toSupabaseReportRow(row) {
  ...
  const supabaseKnownColumns = new Set([
    ...
    'is_public', 'owner_name', 'owner_username', 'owner_department', 'shared_at',
    'note_html', 'note_attachments', 'version', 'created_at', 'updated_at'
  ]);
  const sanitized = {};
