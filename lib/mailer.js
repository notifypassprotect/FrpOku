...
function createMailTemplates({ appName = 'FrpOku', baseUrl = '' } = {}) {
  ...
  const baseHeader = (title = appName) => `
    <div style="background:linear-gradient(135deg, #0f172a 0%, #1e1b4b 50%, #1e3a8a 100%);color:#ffffff;padding:22px 28px;display:flex;align-items:center;justify-content:space-between;border-radius:16px 16px 0 0;border-bottom:2px solid rgba(56,189,248,0.3);">
      <div style="display:flex;align-items:center;gap:12px;">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128" width="36" height="36" style="vertical-align:middle;">
          <rect x="4" y="4" width="120" height="120" rx="28" fill="#0b1120" stroke="rgba(56,189,248,0.4)" stroke-width="2.5" />
          <path d="M 34 26 L 76 26 L 94 44 L 94 98 A 6 6 0 0 1 88 104 L 40 104 A 6 6 0 0 1 34 98 Z" fill="#2563eb" stroke="rgba(255,255,255,0.35)" stroke-width="1.5" />
          <path d="M 76 26 L 76 40 A 4 4 0 0 0 80 44 L 94 44 Z" fill="#38bdf8" />
          <rect x="44" y="40" width="24" height="4.5" rx="2.25" fill="#ffffff" />
          <rect x="44" y="52" width="40" height="3.5" rx="1.75" fill="#06b6d4" />
          <rect x="44" y="61" width="34" height="3.5" rx="1.75" fill="#cbd5e1" />
          <rect x="44" y="70" width="26" height="3.5" rx="1.75" fill="#94a3b8" />
        </svg>
        <span style="font-size:22px;font-weight:800;letter-spacing:-0.5px;color:#ffffff;">${escapeHtml(appName)}</span>
      </div>
      <div style="font-size:12px;font-weight:700;background:rgba(56,189,248,0.15);color:#38bdf8;border:1px solid rgba(56,189,248,0.3);padding:5px 14px;border-radius:20px;">Kurumsal Portal</div>
    </div>
  `;
