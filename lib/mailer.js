    ...
    <div style="background:#ffffff;border:1px solid #e2e8f0;border-radius:16px;overflow:hidden;box-shadow:0 8px 24px rgba(15,23,42,.08);">
      ...
      <div style="padding:28px;line-height:1.65;">
        ...
        <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:18px 20px;margin:20px 0;">
          ...
          <div style="font-size:13.5px;color:#64748b;margin-bottom:6px;">Yeni Giriş Şifreniz:</div>
          <div style="font-family:'Courier New',Courier,monospace;font-size:22px;font-weight:900;letter-spacing:2px;color:#2563eb;background:#eff6ff;border:1.5px dashed #3b82f6;padding:10px 18px;border-radius:8px;display:inline-block;">
            ${newPassword}
          </div>
...
function createMailer({ env = process.env, transporter = null, fetchImpl = globalThis.fetch, logger = console } = {}) {
  ...
  return {
    ...
    sendPasswordResetByAdmin: ({ to, fullName, username, newPassword } = {}) =>
      sendTemplate('passwordResetByAdmin', { to, data: { fullName, username, newPassword } }),
